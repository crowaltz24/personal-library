...
Initial backend for scanning book barcodes and looking up book metadata by ISBN through Open Library.
...
Google Books is used as a fallback when Open Library does not have an ISBN. It works with the public API during development, but you can optionally configure a Google Books API key:

```powershell
$env:GOOGLE_BOOKS_API_KEY = "your-key-here"
```

Alternatively, create a local ignored `.env` file containing `GOOGLE_BOOKS_API_KEY=your-key-here`.

Do not commit API keys. A key previously shared in chat should be revoked or rotated in Google Cloud.
...
Open Library responses are mocked in the tests, so the suite does not require network access.